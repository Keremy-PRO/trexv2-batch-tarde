...
function setup() {// configuración del juego
  ...
  trex = createSprite(50,height-80, 10, 30);
  ...
  pisoInvisible = createSprite(width/2,height-60,width,12);//Se crea un piso falso para que el trex camine sobre el
  pisoInvisible.visible = false;// Hago invisible el piso invisible
  
  gameOver = createSprite(width/2, height/2 -50, 100, 70);
  gameOver.addImage(gameOverImage);
  gameOver.scale = 0.7;
  gameOver.visible = false;


  reinicio = createSprite(width/2, height/2,100,100);
...
function draw() {//Dibujar
  background("white");
  drawSprites();
  text("score: "+ puntos,width-120,20);
  ...
  if (gameState == PLAY) { // Si el estado del juego es play
    ...
    //sonido de perdiste
    if(mousePressedOver(reinicio) || touches.length>0){

      reiniciarJuego();//Mandamos a llamar a la funcionalidad
      touches=[];// Los touches se resetean
    }
  }
...
function crearNubes() { // Funcionalidad que crea nubes
  ...
  if (frameCount % 50 === 0) { //Generar nube cada 50 cuadros de animación
    ...
    nube = createSprite(width+20, height-300 - altura, 30, 10);
...
function crearObstaculos() {
  if (frameCount % 50 === 0) {
    var cactus = createSprite(width, height-95, 20, 70);
